...
export default function LivePostFromSeller() {
  ...
  useEffect(() => {
    ...
    fetchData();
  }, [page, rowsPerPage, filterCategory, filterGrade, filterStatus, filterDate]);
    ...
    <>
      {loading ? (
        ...
        <Box sx={{ mt: 10, ml: { xs: 2, sm: 27 }, mr: 2, pl: 1, pr: 1, width: "auto", borderRadius: "5px" }}>
          ...
          <Typography variant="h6" component="h2" sx={{ fontWeight: "bold", fontSize: "25px", mb: 2 }}>Live Posts From Sellers</Typography>
          <Box sx={{ display: "flex", alignItems: "start", mt: 2, mb: 2, flexDirection: { xs: "column", sm: "row" } }}>
            ...
            <Box sx={{ display: "flex", flexDirection: "row" }}>
              <Button sx={{ ml: 1, mt: { xs: 2, sm: 2 } }} variant="contained" color="secondary" onClick={handleClearFilters}>
                Clear
              </Button>
            </Box>
          </Box>
          <Box>
            <Grid container spacing={2}>
              {requirementData.length === 0 ? (
                <Box sx={{ display: "flex", ml: { xs: 2, sm: 30 } }}>
                  <Typography
                    sx={{
                      p: 10,
                      color: "#B8390E",
                          ...
                          <form onSubmit={(e) => BidNewData(e, bid_amount)}>
                            <Grid container spacing={1}>
                              ...
                              <Grid item xs={(row.status !== "Live") ||
                                ...
                                <Link to={"/buyeruserallbids/" + row.name} style={{ textDecoration: "none" }}><Button variant="outlined" size='small' color="success">All Bids</Button></Link>
                              </Grid>
