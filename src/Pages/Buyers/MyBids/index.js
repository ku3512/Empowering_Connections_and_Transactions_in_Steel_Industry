...
const BuyerBids = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetch(`https://steel.smartyerp.in/api/method/steel.api.get_seller_best_bid?user=${name}&type=${type}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const result = await fetch(`https://steel.smartyerp.in/api/method/steel.api.get_seller_other_bid?user=${name}&type=${type}`, {
          headers: {
            "Content-Type": "application/json",
            "Authorization": `token ${key}:${secret}`,
          }
