...
const RoundPipe = () => {
    ...
    const handleChange = (e) => {
        const { name, value } = e.target;
        if (name === 'thickness' || name === 'length_value') {
            // Check if the input value is a valid positive number
            const regex = /^\d*\.?\d{0,2}$/;
            if (regex.test(value) || value === '') {
                setRequiredForm({ ...RequiredForm, [name]: value });
        ...
        fetch(`https://steel.smartyerp.in/api/method/steel.api.get_single_requirement_for_edit?name=${name}`, {
            ...
            .then(data => {
                ...
                setRequiredForm(prevState => ({
                    ...
                    nb_od:data.data[0].nb_od,
                    erw_seamless:data.data[0].erw_seamless,
                            ...
                            <FormControl>
                                ...
                                <Select
                                 disabled={docStatus === 1 || status === "Withdrawn"}
                                    ...
                                    name="nb_od"
                                    value={RequiredForm.nb_od} // Use selected_expiration_date from state
                            ...
                            <FormControl>
                                ...
                                <Select
                                 disabled={docStatus === 1 || status === "Withdrawn"}
                                    ...
                                    id="demo-simple-select-autowidth"
                                    name="erw_seamless"
                                    value={RequiredForm.erw_seamless} // Use selected_expiration_date from state
                                    onChange={handleChange}
                                    label="ERW/Seamless"
                                >
                                    <MenuItem value="ERW">ERW</MenuItem>
